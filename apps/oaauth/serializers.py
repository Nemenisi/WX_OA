...
class LoginSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if email and password:
            ...
            if not user:
                raise serializers.ValidationError('The mail does not exist. Please contact the administrator for registration.')
            elif not user.check_password(password):
                raise serializers.ValidationError('The password is incorrect.')
            elif user.status == UserStatusChoices.UNACTIVE:
                raise serializers.ValidationError('The user does not active.')
            elif user.status == UserStatusChoices.LOCKED:
                raise serializers.ValidationError('The user has been locked. Please contact the administrator')
            attrs['user'] = user
        else:
